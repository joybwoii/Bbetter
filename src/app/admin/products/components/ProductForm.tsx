  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      if (file.size > 1048576) { // 1MB limit
        alert("The image size is too high, please upload an image under 1MB in size.");
        ...
        <div style={{ gridColumn: 'span 2' }}>
          ...
          {imageSource === 'upload' ? (
            <div className="uploadZone" onClick={() => document.getElementById('fileInput')?.click()}>
              ...
              <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: '0.25rem' }}>Supports PNG, JPG, JPEG, WEBP, SVG (Max 1MB)</p>
              <input 
                type="file" 
                id="fileInput" 
                accept="image/*" 
